...
你是这个项目的资深前端工程协作者，负责把 `docs/home/home.md` 转化为首页高保真原型。
...
| `docs/home/home.md` | 首页内容、模块顺序、CTA、交互和高保真布局的页面级来源。 |
...
9. Talent Lite Band：人才网络轻入口
10. Final Enterprise CTA：企业主入口
11. Footer
...
- `进入商业资源中心` -> `/resources/case-studies`
...
### 5.9 Talent Lite Band Component

目标：给高阶人才一个正式但轻量的入口，参考 BTG 首页中段人才提示的处理方式，不抢企业主路径。

布局：

- 放在 Resource And Case Duo 之后、Final Enterprise CTA 之前。
- Desktop：左侧一句提示，右侧一个 Secondary Button。
- Mobile：文字在上，按钮在下。
- 使用白底、细上下边框，不做大标题区，不使用深色 CTA 背景。

内容：

- 文案：`你有能服务企业关键阶段的专业经验吗？`
- CTA：`加入左安人才网络` -> `/join`
...
- 居中标题 + 一段话 + 企业主按钮。
- 使用 `inverse-surface` 深色背景，作为页面主要收口。
- Final CTA 只服务企业端，避免和人才入口混在同一个 CTA 区块。
...
- 页面是否有足够设计感：图文节奏、tabs 矩阵、资源与案例双入口、Final CTA 都应像真实官网模块。
