...
3. Editorial Section 和 Resource Banner 必须提供图文节奏，避免全页都是卡片。
4. Footer 和 Header 必须像完整网站壳层，而不是临时占位。
5. 其他说明模块可以简洁，不要把每个 section 都做得同样重。

容易干扰高保真的点，开发时要主动避免：

- 不要把所有内容都做成同款卡片网格。Problem Grid、Capability Matrix、Trust Grid 必须有不同视觉结构。
...
- 不要平均分配版面权重。Hero、Capability Matrix、Resource Banner、Final CTA 是视觉重点。

模块顺序：

1. Global Header
2. Hero Split Section
3. Audience + Capability Strip
4. Editorial Section：为什么现在需要阶段性高阶能力
5. Problem Card Grid：企业常见关键阶段问题
6. Process Steps：左安门如何工作
7. Interactive Capability Matrix：我们做什么
8. Trust Feature Grid：为什么选择左安门
9. Resource Feature Banner：资源/洞察入口
10. Talent CTA Band：人才侧入口
...
### 5.3 Audience + Capability Strip Component
...
- 背景：`surface-muted`。
- 横向 3-4 列，移动端堆叠。
- 组件：Label Row / Thin Divider / Text Cell。

内容：

- 服务对象：`成长型企业 / 创业公司 / 投资机构与被投企业 / 转型期组织`
- 能力类型：`分数制高管 / AI 与战略顾问 / 项目型专家团队 / 专家诊断`
...
### 5.4 Editorial Section Component
...
`企业缺的往往不是更多人，而是能判断问题、进入现场、推动结果的人`

正文：

`在 AI 转型、增长瓶颈、组织变化和关键项目推进中，企业经常遇到的不是单点人手不足，而是缺少能快速理解业务、定义任务并带动执行的高阶能力。`
...
### 5.5 Problem Card Grid Component
...
- Tablet：2 列。
- Mobile：1 列。
- 组件：Line Icon + Card Title + 2-line Description + Text Link。
...
2. `AI 转型落地`
   - 描述：`不只是选工具，而是判断哪些业务环节值得先改。`
3. `组织升级`
   - 描述：`企业进入新阶段，需要阶段性的管理能力和组织设计支持。`
4. `临时负责人缺口`
   - 描述：`CFO、COO、CMO、HRVP 等关键角色暂时缺位，需要过渡方案。`
5. `新业务探索`
   - 描述：`需要外部高阶人才帮助验证方向、搭建路径、推进试点。`
6. `项目攻坚`
   - 描述：`关键项目缺少负责人、PMO 或跨职能专家组合。`
...
### 5.7 Interactive Capability Matrix Component
...
- `分数制高管`
  - 描述：`阶段性 CFO / COO / CMO / HR / CTO 等高阶角色支持。`
  - Link：`/services`
- `AI 与战略顾问`
  - 描述：`判断 AI 转型机会、业务流程改造和战略选择。`
  - Link：`/services`
- `项目型专家团队`
  - 描述：`为增长、组织、转型、新业务等具体项目组建小型专家组。`
  - Link：`/services`
- `专家诊断与访谈`
...
### 5.8 Trust Feature Grid Component
...
- 2 x 2 feature cards。
- 背景可使用白色或 `surface-muted`。
- 每张卡使用细边框，不使用重阴影。

内容：

1. `先定义问题，再匹配人`
   - `左安门不只做人才撮合，而是先帮助企业把问题、任务和结果边界说清楚。`
2. `高阶人才，而非普通 freelancer`
   - `重点连接有管理经验、项目经验或专业判断力的独立人才。`
3. `灵活合作方式`
   - `可根据问题选择分时、项目制、顾问陪跑或专家团队。`
4. `面向企业使用场景`
   - `关注需求澄清、合作边界、过程跟进和基本合规，而不是只完成介绍。`
...
### 5.9 Resource Feature Banner Component

目标：像 BTG report block 一样，用内容资产证明左安门有问题判断能力。
...
`一份面向企业经营者的简明指南：什么时候适合找独立高管、顾问或专家团队，什么时候更适合全职招聘或传统咨询。`

CTA：

- `阅读资源` -> `/resources`

### 5.10 Talent CTA Band Component
...
`左安门欢迎有真实业务经验、专业判断力和项目交付能力的独立人才。`

CTA：

- `申请入席` -> `/join`

### 5.11 Final Enterprise CTA Component
...
- 让首页像完整官网，而不是单页 demo。
- 提供清晰的站点导航、企业入口、人才入口和基础法律文本占位。

布局：

- Desktop：4 列 footer + bottom bar。
- Tablet：2 列。
- Mobile：单列纵向排列。
...
- 标题：`左安门`
- 简介：`为企业关键阶段连接高阶独立人才。`
- 可选小字：`阶段性高阶能力支持，面向增长、组织、AI 转型和关键项目。`
...
- 不要在 footer 中放未确认地址、公司实体、备案号、电话或邮箱。
...
flowchart TD
  ...
  B --> C[Audience + Capability Strip: 服务对象 + 能力类型]
  C --> D[Editorial Section: 为什么现在需要阶段性高阶能力]
  D --> E[Problem Card Grid: 6 个企业关键阶段问题]
  E --> F[Process Steps: 提交需求 -> 定义任务 -> 连接人才]
  F --> G[Interactive Capability Matrix: Tabs + 可点击能力卡片]
  G --> H[Trust Feature Grid: 为什么选择左安门]
  H --> I[Resource Feature Banner: 企业使用阶段性高阶人才指南]
